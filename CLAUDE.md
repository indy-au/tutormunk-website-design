...
## Topic pages (added 19 Aug 2026, content pass batches 1 and 2 of 3)

- 27 pages live under /topics/. 11 are done with full, individually written content: primary-maths, selective-school-prep, oc-prep, naplan, writing-program (batch 1), plus high-school-english, high-school-maths, high-school-science, naplan-years-7-and-9, icas, hsc-sprint (batch 2). The other 16 (the 16 HSC/senior subject pages: english-studies, english-standard, english-advanced, english-extension-1, english-extension-2, english-eal-d, maths-standard, maths-standard-1, maths-standard-2, maths-extension-1, maths-extension-2, biology, chemistry, physics, earth-and-environmental, investigating-science) still use the shorter shared `topic()` helper shell in src/content/topics.ts and render only Hero, DeliveryModesBand, CtaBand, exactly as before this pass. Batch 3 must bring these 16 up to the same shape, following the rules below.
- `TopicContent` in src/content/topics.ts gained five optional fields on top of the original ones (title, metaDescription, eyebrow, heading, intro, heroImage, all still required): `targetKeywords: string[]`, `entitySentence: string`, `sections: { heading: string; paragraphs: string[] }[]`, `faqs: { question: string; answer: string }[]`, `related: { label: string; to: string }[]`. A topic with all five renders the richer page (src/routes/topics.$slug.tsx): intro block with `entitySentence` as the first paragraph then `sections` as h2/paragraph blocks (TopicIntro.tsx), the FAQ list with FAQPage JSON-LD (FaqAccordion.tsx + FaqSchema.tsx, both already generic, reused unchanged), a Related links row (TopicRelated.tsx), on top of the unchanged Hero/DeliveryModesBand/CtaBand. A topic without all five fields renders exactly as before. Keep using `topic()` for anything not yet rewritten so it keeps compiling; write full literal objects for anything getting the real content pass, it reads better than burying prose in helper arguments.
- Content rules for every topic page, restated short: in-person only, never "online"/"virtual"/"Zoom", the delivery line is "in your home or at our Gregory Hills centre"; "NESA aligned" never "NESA accredited"; invent nothing, no prices, tutor counts, results, guarantees, school names, statistics, years in business, "award-winning", only claims already on the site or well-established public syllabus/test facts; Australian English, no em or en dashes, no exclamation marks, no keyword stuffing; tutors referenced as first name plus surname initial only, never a full name; every page's body text must be genuinely unique, no shared sentences between pages (checked with a real sentence-level diff, not by eye) except the delivery line itself; roughly 350 to 450 words of unique body text per page excluding FAQs; FAQ answers 40 to 80 words, 3 to 4 real parent questions per page; titles under 60 characters with the primary phrase first and "| TutorMunk" last, metaDescriptions 120 to 155 characters opening with "TutorMunk" and the service and mentioning in person and suburban Sydney or the centre.
- Plain English rule (added batch 2, 19 Aug 2026, applies to all topic content from here on): write like a careful teacher explaining to a parent, concrete over abstract, short sentences fine. No em dashes, no exclamation marks, no rhetorical questions as headings, no sentence beginning "Whether you're..." or "In today's...", no paragraph opening with the same word as the paragraph before it, no lists of three adjectives, no "not just X but Y" constructions. Banned words and phrases, checked with a real grep every batch (see scripts/report-topics-batch2.mjs), must come back zero: unlock, unleash, journey, tailored, bespoke, empower, elevate, seamless, holistic, foster, nurture, thrive, transform, transformative, robust, leverage, passion, passionate, dive into, delve, navigate, landscape, game-changer, cutting-edge, world-class, it's important to note, it is worth noting, in essence, ultimately, at the end of the day, a testament to, ensure (use "make sure"), utilise/utilize (use "use"), crucial, vital, essential (as emphasis adjectives), comprehensive, dedicated, personalised/personalized (the site uses "matched" and "one to one").
- Measure, don't eyeball, before calling a page done: word counts, FAQ answer lengths, title/description character counts, the banned-word grep and the cross-page sentence-duplicate check all need an actual script run (see scripts/report-topics-batch1.mjs and scripts/report-topics-batch2.mjs for the pattern, both load the real content module via Vite SSR rather than hand-counting), per the "How prompts for Claude Code are written" measurement culture below. The duplicate check must run across every content-complete page done so far, not just the batch in progress, since a new page can collide with an older one.
- When a shared route or content file that every topic page depends on changes (like topics.$slug.tsx gaining new imports, or topics.ts itself growing new entries), the built HTML for untouched topic pages will show a different JS chunk hash and preload list in the page's embedded hydration script, that is a normal Rollup code-splitting side effect, not a content regression. The actual visitor and Google facing markup (everything before that inline hydration `<script>`) should stay byte-identical, confirm that specifically if verifying "nothing else changed" after a future batch.
