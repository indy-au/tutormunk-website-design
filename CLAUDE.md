...
## Form endpoint (added 18 Aug 2026, Phase 3B)

- Request a Call actually sends somewhere now. The PHP endpoint lives at deploy/php/send.php, packaged into dist-static/send.php (site root) by `npm run build:static` alongside deploy/.htaccess, deploy/php/PHPMailer/ and deploy/php/mail.config.example.php. PHPMailer is vendored (no Composer on Hostinger), version pinned in a comment at the top of send.php.
- deploy/php/mail.config.php holds the real Titan mailbox password and is never committed (confirmed with `git check-ignore`) and never packaged into dist-static/, even if it exists locally for testing. On the server it is created by hand from mail.config.example.php, see deploy/DEPLOY.md. send.php refuses to run (500, logged) if it is missing or the password is empty, so a forgotten password fails loudly instead of silently losing leads.
...
## Topic pages (added 19 Aug 2026, content pass complete, all 3 batches)

- All 27 pages under /topics/ now have full, individually written content. Batch 1: primary-maths, selective-school-prep, oc-prep, naplan, writing-program. Batch 2: high-school-english, high-school-maths, high-school-science, naplan-years-7-and-9, icas, hsc-sprint. Batch 3: english-studies, english-standard, english-advanced, english-extension-1, english-extension-2, english-eal-d, maths-standard, maths-standard-1, maths-standard-2, maths-extension-1, maths-extension-2, biology, chemistry, physics, earth-and-environmental, investigating-science. The `topic()` helper in src/content/topics.ts is unused by any live entry now (every one of the 27 is a full literal object) but is kept in place, not deleted, as the pattern for any future topic page. Any new topic page should follow the same rules below from the start, using a full literal object, not the `topic()` shell.
- `TopicContent` in src/content/topics.ts has five optional fields on top of the original ones (title, metaDescription, eyebrow, heading, intro, heroImage, all still required): `targetKeywords: string[]`, `entitySentence: string`, `sections: { heading: string; paragraphs: string[] }[]`, `faqs: { question: string; answer: string }[]`, `related: { label: string; to: string }[]`. A topic with all five renders the richer page (src/routes/topics.$slug.tsx): intro block with `entitySentence` as the first paragraph then `sections` as h2/paragraph blocks (TopicIntro.tsx), the FAQ list with FAQPage JSON-LD (FaqAccordion.tsx + FaqSchema.tsx, both already generic, reused unchanged), a Related links row (TopicRelated.tsx), on top of the unchanged Hero/DeliveryModesBand/CtaBand.
- Content rules for every topic page, restated short: in-person only, never "online"/"virtual"/"Zoom", the delivery line is "in your home or at our Gregory Hills centre"; "NESA aligned" never "NESA accredited"; invent nothing, no prices, tutor counts, results, guarantees, school names, statistics, years in business, "award-winning", ATAR claims, only claims already on the site or well-established public syllabus/test facts, and if uncertain of a specific detail (a module name, a course structure), use safer general phrasing and say so in the delivery report rather than guessing; Australian English, no em or en dashes, no exclamation marks, no keyword stuffing; tutors referenced as first name plus surname initial only, never a full name; every page's body text must be genuinely unique, no shared sentences between any of the 27 pages (checked with a real sentence-level diff, not by eye) except the delivery line itself; roughly 350 to 450 words of unique body text per page excluding FAQs; FAQ answers 40 to 80 words, 3 to 4 real parent (and, for the senior pages, student) questions per page; titles under 60 characters with the primary phrase first and "| TutorMunk" last, metaDescriptions 120 to 155 characters opening with "TutorMunk" and the service and mentioning in person and suburban Sydney or the centre.
- Plain English rule (added batch 2, extended batch 3, 19 Aug 2026, the standing rule for any future topic content): write like a careful teacher explaining to a parent, and for the senior pages, to the Year 11 or 12 student who will read it themselves, no talking down. Concrete over abstract, short sentences fine. No em dashes, no exclamation marks, no rhetorical questions as headings, no sentence beginning "Whether you're..." or "In today's...", no paragraph opening with the same word as the paragraph before it, no lists of three adjectives, no "not just X but Y" constructions, no ATAR pressure language. Banned words and phrases, checked with a real grep every batch (see scripts/report-topics-batch3.mjs, the current version of the checker), must come back zero: unlock, unleash, journey, tailored, bespoke, empower, elevate, seamless, holistic, foster, nurture, thrive, transform, transformative, robust, leverage, passion, passionate, dive into, delve, navigate, landscape, game-changer, cutting-edge, world-class, it's important to note, it is worth noting, in essence, ultimately, at the end of the day, a testament to, ensure (use "make sure"), utilise/utilize (use "use"), crucial, vital, essential (as emphasis adjectives), comprehensive, dedicated, personalised/personalized (the site uses "matched" and "one to one"), mastery, master (as a verb), excel, ace, top marks, skyrocket, boost.
- Measure, don't eyeball, before calling a page done: word counts, FAQ answer lengths, title/description character counts, the banned-word grep and the cross-page sentence-duplicate check all need an actual script run (see scripts/report-topics-batch3.mjs, which supersedes the batch1/batch2 versions, covers the full banned list, and dynamically discovers every content-complete page rather than a hardcoded list), per the "How prompts for Claude Code are written" measurement culture below. The duplicate check must run across every content-complete page, not just the batch in progress, since a new page can collide with an older one. Watch especially for sibling courses (Standard 1 vs Standard 2, Extension 1 vs Extension 2, the sciences): the shared facts (60 minutes, capped at six, written note, Band 6 tutor) are the most likely place an identical sentence creeps in across many pages at once, it takes real, deliberately varied wording each time, not a synonym swap.
- When a shared route or content file that every topic page depends on changes (like topics.$slug.tsx gaining new imports, or topics.ts itself growing new entries), the built HTML for other topic pages will show a different JS chunk hash and preload list in the page's embedded hydration script, that is a normal Rollup code-splitting side effect, not a content regression. The actual visitor and Google facing markup (everything before that inline hydration `<script>`) should stay byte-identical, confirm that specifically if verifying "nothing else changed" after a future content edit. A raw HTML grep for "!" will also show 11 hits per page from React's SSR hydration markers and minified JS booleans (`!0`, `!1`, `<!--$-->`), unrelated to the actual copy, always check exclamation marks at the field level (title, metaDescription, heading, intro, entitySentence, section text, FAQs) not the raw built file.
- EXTENSION COURSES ARE NOT GRADED IN BANDS (corrected 19 Aug 2026). HSC Extension courses only, English Extension 1 and 2, Mathematics Extension 1 and 2, are reported on an E1 to E4 scale, not Bands 1 to 6. The "senior tutors achieved a Band 6 in the course they teach" claim (seniorSchool.ts, and restated on every other topic page) does not hold for these four specifically: their pages instead say the tutor achieved a Band 6 in English Advanced (the two English Extension pages) or in Mathematics Advanced (the two Maths Extension pages), never a Band claim about the Extension course itself, and never an E4 claim either, since that has not been verified as an established site fact. Keep this in mind for any future copy touching these four courses.

## Suburb pages (added 19 Aug 2026, content pass batch 4)

- All 23 suburb pages under src/content/locations.ts (rendered by SuburbPage.tsx) now carry the same content shape as the topic pages, layered onto the existing heroIntro and two bodySections, which stayed in place, this was an upgrade, not a rewrite. `SuburbLocation` gained two optional fields: `entitySentence: string`, rendered as the first paragraph under the hero (TopicIntro.tsx, whose `sections` prop is now optional and defaults to `[]` so a suburb page can reuse it for just the one paragraph, no h2 blocks); and `faqs: { question; answer }[]`, three per suburb, rendered with FaqAccordion + FaqSchema (FAQPage JSON-LD) below the delivery section and above the reviews carousel. `BodySection` (used by all three body sections) gained an optional `links?: { label; to }[]`, used only on the new third section, rendered as a small pill row underneath it (TopicRelated.tsx, reused unchanged from the topic pages), max three links, real routes only.
- The third bodySection on each page (`suburb.bodySections[2]`, rendered by SuburbPage.tsx alongside the first two) is 90 to 140 words of suburb-specific content built around that page's own targetKeywords and which subjects the site actually offers, primary English and maths, Years 7 to 10 English/maths/science, HSC courses, Selective, OC, NAPLAN, ICAS, HSC Sprint, linked out to the real topic page where natural. Which subject leads varies deliberately by suburb (some pages lead with primary, some high school, some HSC, some general exam prep), so the section reads differently page to page rather than following one template.
- NO NEW PLACE FACTS, ever, on a suburb page: the only geographic fact allowed is the suburb name itself and whatever that page already said before this pass, no school names, landmarks, train stations, distances, or invented local colour. The only delivery facts allowed are the fixed list: in-home sessions; the Gregory Hills centre at The HUB, 31 Lasso Road, open by appointment; 60-minute sessions; one to one or small group; WWCC-verified tutors; a written note after every session; centre groups capped at six in the same stage. FAQ answers in particular must stick to only these facts, since a parent question is an easy place to accidentally invent a detail (opening hours, distances, minimum booking lengths) that was never established.
- The single hardest part of this batch, worth remembering for any future suburb copy: the 23 pages share the same narrow fact set, which makes templated sentences (the same sentence shape with only the suburb name swapped) the path of least resistance, and it takes real, deliberate structural variation to avoid it, not a find-and-replace of the suburb name. A first pass at this batch produced over 500 masked-pattern duplicates (checked by replacing the suburb name with a placeholder and comparing sentences across all 23) before individual rewriting brought it to zero, concentrated in exactly the sentences that restate a fixed fact: the entitySentence opener, the "can we use the centre" FAQ, the "which subjects" FAQ, and the "capped at six" line. Vary the sentence structure itself (verb, subject, clause order), not just individual words, and rerun the checker (scripts/report-locations-batch4.mjs) after every batch of edits, not once at the end.
- Hub page heroes (primarySchool.ts, highSchool.ts, seniorSchool.ts, examPrep.ts) now open with one entity sentence prepended to `hero.body`, written individually per stage, stating what TutorMunk provides, for whom, in person, across suburban Sydney, in the family home or at the Gregory Hills centre, with the existing body text kept unchanged after it. Confirmed at 1440x900 the CTA button stays comfortably above the fold on all four (roughly 30 to 70px of clearance under the 900px viewport height) and at 375 nothing overflows horizontally.

## Blog (added 20 Aug 2026, risk and quality remediation)

- The blog was migrated from the owner's other business, Sprouts Academy, and the migrated set was mostly unreviewed. A full audit quarantined every migrated post that could not pass the same standards as the rest of the site, rather than deleting anything. Nothing was deleted: a quarantined post's frontmatter, full body, links and `sourceUrl` are all preserved exactly, only `draft: true` was added or flipped. See CLAUDE-blog-remediation-report.md (this repo root) for the full measured report: baseline counts, the retained list with reasons, the quarantined list grouped by reason, and every validation result. Do not treat that report's counts as a standing fact to quote elsewhere, it is a dated measurement, not a rule; re-measure before citing a figure again.
- Publication is a strict, single field: a post is published only when its frontmatter has no `draft` field, or `draft: false`. It is a draft, and excluded from every public surface, only when the frontmatter literally contains `draft: true` (checked in src/lib/blogLoader.ts as `data["draft"] === "true"`, case-sensitive string comparison after quote-stripping). `publishedBlogPosts` in blogLoader.ts is the single choke point every public surface reads from (individual post routes, the blog index, pagination, category filtering, related posts, the sitemap), so correctly setting this one field is what keeps a post off the site, nothing else needs separate updating for that part.
- A published blog post must meet the same standards as every other page on this site: in-person only, never online tutoring or a hint of it; no guarantees, promised results or unsupported outcome/performance/testimonial claims; "NESA aligned" never "NESA accredited"; the same banned-word list as the rest of the site (see "Plain English rule" above); Australian English, no em or en dashes, no exclamation marks; a genuinely working set of internal links (verified against the actual build, not assumed); a real image at the path its frontmatter names. A migrated post's own text is not evidence for a claim it makes about TutorMunk, current site content and authoritative primary sources are.
- A quarantined post must not be republished by simply flipping `draft` back to `false`. It needs the same full pass a newly retained post gets: claims and voice review against current site standards, a working internal-link and image check, and a fact check against a primary source for anything time-sensitive (NAPLAN, Selective, OC, HSC, NESA process). Never republish on the strength of the post having been "fine before".
- Dates: `date` is the only date field this schema has. It is mapped to the BlogPosting JSON-LD `datePublished` (BlogPostingSchema.tsx, wired into blog.$slug.tsx) because it is already the date shown on every post page. Never invent or backfill a date from git history, a file timestamp, or the migration date, and never stamp today's date onto a post just because its frontmatter was edited. If a post has no reliable date, omit `datePublished` rather than guess. Attribution on every post is the fixed line "Reviewed and updated by the TutorMunk team" (BlogArticle.tsx), never a named, invented author.
- `sourceUrl` in a migrated post's frontmatter is provenance only, so the original Sprouts article can be found again if needed. It must never be used as the canonical or `mainEntityOfPage` URL, that is always the live tutormunk.com.au blog URL, and it must never appear as a rendered link on the page.
- Before publishing any new or previously-quarantined post, it needs to pass: the claims and banned-word review above; every internal link and local image reference checked against the actual build output; the rendered BlogPosting JSON-LD parsed and checked for empty or fabricated fields; and a normal `npm run build:static` with no failures.
