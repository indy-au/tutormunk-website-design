...
## Topic pages (added 19 Aug 2026, content pass batch 1 of 3)

- 27 pages live under /topics/. 5 are done with full, individually written content: primary-maths, selective-school-prep, oc-prep, naplan, writing-program. The other 22 (high-school-*, the HSC subject pages, icas, hsc-sprint, naplan-years-7-and-9 etc.) still use the shorter shared `topic()` helper shell in src/content/topics.ts and render only Hero, DeliveryModesBand, CtaBand, exactly as before this pass. Batches 2 and 3 must bring the remaining 22 up to the same shape, one at a time or in further batches, following the rules below.
...
- Measure, don't eyeball, before calling a page done: word counts, FAQ answer lengths, title/description character counts and the cross-page sentence-duplicate check all need an actual script run (see scripts/report-topics-batch1.mjs for the pattern, it loads the real content module via Vite SSR rather than hand-counting), per the "How prompts for Claude Code are written" measurement culture below.
- When a shared route or content file that every topic page depends on changes (like topics.$slug.tsx gaining new imports), the built HTML for untouched topic pages will show a different JS chunk hash and preload list in the page's embedded hydration script, that is a normal Rollup code-splitting side effect, not a content regression. The actual visitor and Google facing markup (everything before that inline hydration `<script>`) should stay byte-identical, confirm that specifically if verifying "nothing else changed" after a future batch.
